$(function() {
	...
	$('.delete').on('click', function() {
		...
		var userId = tr.find('#td-id').text().trim();
		$('#form-hide').find('#userId').val(userId);
		bootbox.dialog({
			message : "确定要删除吗?",
			title : "注意",
			buttons : {
	...
	$('.edit')
			.on(
					...
					function() {
						...
						var tr = $(this).parent().parent();
						$('#modal-edit').find('#role-name1').val(
								tr.find('#td-fullName').text());
						$('#modal-edit').find('#role-jobNumber').val(
								tr.find('#td-jobNumber').text());
						$('#modal-edit').find('#role-Phone').val(
								tr.find('#ff').text());
						$('#modal-edit').find('#role-email').val(
								tr.find('#gg').text());
						$('#modal-edit').modal('show');
					});
